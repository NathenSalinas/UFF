...
public:
	...
	{
		if (!strcmp(event->GetName(), "bullet_impact") && Menu::Window.VisualsTab.BulletTracers.GetState() && Menu::Window.VisualsTab.Active.GetState())
		{
	...
	void OnStudioRender()
	{
		if (Menu::Window.VisualsTab.BulletTracers.GetState() && Menu::Window.VisualsTab.Active.GetState()) {
			for (unsigned int i = 0; i < trace_logs.size(); i++) {
				...
				if (shooter->GetTeamNum() == 3)
					color = Color(Menu::Window.ColorsTab.BULLETCT_R.GetValue(), Menu::Window.ColorsTab.BULLETCT_G.GetValue(), Menu::Window.ColorsTab.BULLETCT_B.GetValue(), 255);
				else
					color = Color(Menu::Window.ColorsTab.BULLET_T_R.GetValue(), Menu::Window.ColorsTab.BULLET_T_G.GetValue(), Menu::Window.ColorsTab.BULLET_T_B.GetValue(), 255);
